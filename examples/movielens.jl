...
t = readdlm("/Users/madeleine/Documents/papers/convexsketching/ml-100k/u.data", '\t', Int);
A = sparse(t[:,1], t[:,2], t[:,3]);

maxrank = 10
cv_params = FrankWolfeParams(maxiters = 500, reltol = 1e-1, stepsize = BacktrackingStepSize(1, .5, .1));
params = FrankWolfeParams(maxiters = 1000, reltol = 1e-2, stepsize = BacktrackingStepSize(1, .5, .1));
...
reg_params = linspace(4000,5000,11)
...
# 	regularization_path(gfrm_quad, params=cv_params, reg_params=reg_params);
...
gfrm_poisson.r = TraceNormConstraint();

train_error, test_error, train_time, reg_params = 
	regularization_path(gfrm_poisson, params=cv_params, reg_params=reg_params);
@show opt_param = reg_params[indmin(test_error)]
